...
Vinverse<T, mode, eclip, thresh>::Vinverse(PClip child, float sstr, int amnt, int uv, float scl, int opt, PClip clip2, int thr,
    IScriptEnvironment* env)
    : GenericVideoFilter(child), sstr_(sstr), amnt_(amnt), uv_(uv), scl_(scl), clip2_(clip2), thr_(thr), blur3_buffer(nullptr),
    blur6_buffer(nullptr), v8(env->FunctionExists("propShow"))
{
    if (!vi.IsPlanar())
    ...
    if (clip2)
    {
        if (thresh > 0)
        {
            switch (clip->GetVideoInfo().ComponentSize())
            {
                ...
                default:
                    env->ThrowError("Vinverse: only 8..16-bit input is supported!");
                    return AVSValue();
            }
        }
        else
